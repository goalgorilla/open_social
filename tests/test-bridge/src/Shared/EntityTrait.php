...
trait EntityTrait {
  ...
  protected function getEntityIdFromLabel(string $entity_type_id, ?string $bundle, string $label) : ?int {
    ...
    if ($labelKey === FALSE) {
      throw new \RuntimeException("$entity_type_id does not support the label entity key.");
    }

    $query = $storage
      ->getQuery()
      ->accessCheck(FALSE)
      ->condition($labelKey, $label);

    if ($bundle !== NULL) {
      $bundleKey = $definition->getKey('bundle');
      if ($bundleKey === FALSE) {
        throw new \RuntimeException("$entity_type_id does not support the bundle entity key.");
      }
  ...
  protected function validateEntityFields(string $entity_type, array &$values) : void {
    ...
    assert($definition instanceof EntityTypeInterface, "The entity type '$entity_type' does not exist.");
    /** @var ?string $bundle */
    ...
    $dummy = $this->entityTypeManager
      ->getStorage($entity_type)
      ->create($bundle ? [$bundle => $values[$bundle]] : []);
          ...
          $values[$field_name] = array_map(
            function ($id_or_name) use ($group_storage, $allowed_bundles) {
              ...
              if (!is_array($group_ids) || empty($group_ids)) {
                throw new \InvalidArgumentException("Group $id_or_name does not exist within group types: " . implode(", ", $allowed_bundles));
              }
