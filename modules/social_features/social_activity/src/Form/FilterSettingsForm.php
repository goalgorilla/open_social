...
namespace Drupal\social_activity\Form;
 ...
 *
 * @package Drupal\social_activity\Form
 */
class FilterSettingsForm extends ConfigFormBase implements ContainerInjectionInterface {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    // Only show blocks field if there are displays available.
    if (!empty($displays)) {
      ...
      $form['social_activity_filter']['no_blocks'] = [
        '#markup' => '<p>' . $this->t('No activity stream views with block displays found.') . '</p>',
      ];
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    foreach ($blocks as $id => $block) {
      $tag_filter = (bool) $block;
      [$views_id, $display_id] = explode('__', $id);
      $this->updateDisplayBlock($views_id, $display_id, $tag_filter);
    }
  ...
  /**
   * Finds all referenced taxonomy fields.
   *
  ...
  public function updateDisplayBlock(string $views_id, string $display_id, bool $enabled = FALSE): void {
    ...
    // Skip if view config doesn't exist.
    if ($config->isNew()) {
      return;
    }
