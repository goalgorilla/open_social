...
final class EdaHandler {
    ...
    return new CloudEvent(
      ...
      data: [
        'user' => new UserEventData(
          ...
          firstName: $profile->get('field_profile_first_name')->value ?? '',
          lastName: $profile->get('field_profile_last_name')->value ?? '',
          email: $user->getEmail(),
          roles: array_values($user->getRoles()),
          timezone: $user->getTimeZone(),
          language: $user->getPreferredLangcode(),
          address: Address::fromFieldItem(
            item: $profile->get('field_profile_address')->first(),
          ),
          phone: $profile->get('field_profile_phone_number')->value ?? '',
  ...
  private function determineActors(): array {
    ...
    switch ($this->routeName) {
      ...
      case 'system.batch_page.json':
        $user = $this->currentUser;
        break;

      case 'scim.user_create':
      case 'scim.user_put':
        $application = 'scim';
        break;
    }
