...
class SocialUserLoginForm extends UserLoginForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['username_login']['name_or_mail'] = [
      ...
      '#description' => $this->t('Enter your @s username or email.', [
        '@s' => $config->get('name'),
      ]
      ),
      '#required' => TRUE,
    ...
    if (\Drupal::config('user.settings')->get('register') != 'admin_only') {
      ...
      $form['username_login']['sign-up-link'] = [
        '#markup' => $this->t("Don't have an account yet? @link", [
          "@link" => $sign_up_link,
        ]
        ),
        '#weight' => 1000,
  ...
  public function validateNameMail(array &$form, FormStateInterface $form_state) {
    ...
    if ($user_blocked) {
      ...
      if (!$form_state->get('uid')) {
        ...
        $form_state->setErrorByName('name_or_mail', $this->t('The username %name has not been activated or is blocked.', [
