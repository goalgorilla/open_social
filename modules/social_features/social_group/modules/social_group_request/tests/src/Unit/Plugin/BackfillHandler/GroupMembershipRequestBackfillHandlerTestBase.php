...
/**
 * Base test class for group membership request backfill handlers.
 *
 * Provides shared test methods for testing request handlers that differ
 * only by status value and handler method name.
 */
abstract class GroupMembershipRequestBackfillHandlerTestBase extends UnitTestCase {
   ...
   *
   * @return string
   *   The status value (e.g., 'pending', 'approved', 'rejected').
   */
  abstract protected function getExpectedStatusValue(): string;

  /**
   * Gets the expected handler method name.
   *
   * @return string
   *   The handler method name (e.g., 'groupMembershipRequestCreate').
   */
  ...
  public function testProcess(): void {
    ...
    $eda_handler = new class($method_name) {
      ...
      /**
       * The request passed to the handler method.
       */
      public ?GroupRelationshipInterface $requestPassed = NULL;
      ...
      public function __call(string $name, array $arguments): void {
        if ($name === $this->methodName && isset($arguments[0])) {
          ...
          $this->requestPassed = $arguments[0];
        }
      }
    ...
    $this->assertSame($request, $eda_handler->requestPassed);
  }
  ...
  /**
   * Tests getEntityIds() queries group membership requests with correct status.
   */
  public function testGetEntityIds(): void {
    ...
    $this->assertContains(['plugin_id', 'group_membership_request', NULL], $conditions);
    $this->assertContains(['group_type', 'flexible_group', NULL], $conditions);
    $this->assertContains(['grequest_status', $expected_status, NULL], $conditions);
  }
