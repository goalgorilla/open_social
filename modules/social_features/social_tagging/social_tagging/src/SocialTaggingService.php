  ...
  public function getKeyValueOptions(): array {
    ...
    foreach ($types as $entity_type => $item) {
      ...
      if (empty($item) || $entity_type == 'group') {
        $options['tag_type_' . $entity_type] = $definition->getLabel();
        continue;
      }
      foreach ($item as $value) {
        ...
        foreach ($value['bundles'] as $bundle) {
          $bundle_label = $this->entityTypeManager->getStorage($bundle_entity_type)->load($bundle)->label();
          $title = $this->t('@entity_type type: @bundle', [
            '@entity_type' => $label,
            '@bundle' => $bundle_label,
          ]);
          // Key now contains type and bundle. 
          $options["tag_{$entity_type}_type_$bundle"] = $title;
        }
      }
    }
    return $options;
