...
/**
 * Transforms raw GraphQL input into a validated UpdateEventInput object.
 *
 * @DataProducer(
 *   id = "update_event_input",
 *   name = @Translation("Update Event Input Transformer"),
 *   description = @Translation("Transforms raw GraphQL input array into a validated UpdateEventInput wrapper object with injected dependencies."),
 *   produces = @ContextDefinition("any",
 *     label = @Translation("UpdateEventInput")
 *   ),
 *   consumes = {
 ...
 * )
 */
class UpdateEventInput extends DataProducerPluginBase implements ContainerFactoryPluginInterface {

  /**
   * Constructs an UpdateEventInput.
   *
  ...
  /**
   * Transforms raw GraphQL input into an UpdateEventInput wrapper object.
   *
   * @param array $input
   *   The raw input array from the GraphQL mutation request.
   *
   * @return \Drupal\social_event\Wrappers\Input\UpdateEventInput
   *   An UpdateEventInput object populated with the input values and ready
   *   for validation.
   */
  public function resolve(array $input): UpdateEventInputWrapper {
    $event_input = new UpdateEventInputWrapper(
