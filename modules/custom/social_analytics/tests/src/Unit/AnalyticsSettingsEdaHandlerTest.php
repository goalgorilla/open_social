...
/**
 * @coversDefaultClass \Drupal\social_analytics\AnalyticsSettingsEdaHandler
 *
 * @group social_analytics
 */
final class AnalyticsSettingsEdaHandlerTest extends UnitTestCase {
  ...
  /**
   * @covers ::fromDefaultChange
   */
  public function testFromDefaultChangeBuildsExpectedPayload(): void {
    $handler = $this->createHandler();

    $event = $handler->fromDefaultChange(FALSE);

    self::assertSame('com.getopensocial.cms.analytics.settings', $event->getType());
    self::assertSame('/admin/config/opensocial/analytics', $event->getSource());
    self::assertFalse($event->getData()['settings']['showInIndividualMetrics']);
  ...
  /**
   * @covers ::dispatchDefaultChange
   */
  public function testDispatchDefaultChangeDispatchesToAnalyticsTopic(): void {
    ...
    $dispatcher->expects(self::once())
      ...
      ->with(
        ...
        self::callback(static function (CloudEventInterface $event): bool {
          return $event->getType() === 'com.getopensocial.cms.analytics.settings';
        }),
      );

    $handler = $this->createHandler($dispatcher);
    $handler->dispatchDefaultChange(TRUE);
  }

  /**
   * @covers ::dispatchDefaultChange
   */
  public function testDispatchDefaultChangeSkipsWithoutDispatcher(): void {
   ...
   * @covers ::dispatchDefaultChange
   */
  public function testDispatchDefaultChangeSkipsWhenSocialEdaDisabled(): void {
    $dispatcher = $this->createMock(DispatcherInterface::class);
  ...
  private function createHandler(
    ...
    $request->method('getPathInfo')->willReturn('/admin/config/opensocial/analytics');
    ...
    $current_user->method('getRoles')->willReturn(['authenticated', 'sitemanager']);
    $current_user->method('getAccount')->willReturn($actor_user);

    $route_match = $this->createMock(RouteMatchInterface::class);
    $route_match->method('getRouteName')->willReturn('social_analytics.settings');
    ...
    return new AnalyticsSettingsEdaHandler(
      $resolved_dispatcher,
      $module_handler,
      $request_stack,
      $current_user,
