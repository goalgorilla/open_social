 ...
 * }&NodeArray
 */
final class HeadingNode implements NodeInterface {
   ...
   * @param bool $hasLevel
   *   Whether the level field was explicitly provided.
   */
  public function __construct(
    ...
    private readonly bool $hasLevel = TRUE,
  ) {}

  /**
   * Creates a HeadingNode from an array.
   *
  ...
  public static function fromArray(array $data, NodeFactory $factory): self {
    ...
    $hasLevel = isset($data['level']) && is_int($data['level']);
    $level = 1;
    if ($hasLevel) {
      /** @var int $levelValue */
    ...
    $knownFields = ['type', 'version', 'level', 'children'];
    $unknownFields = array_diff_key($data, array_flip($knownFields));

    return new self($version, $level, $children, $unknownFields, $hasChildrenKey, $hasVersion, $hasLevel);
  }
  ...
  /**
   * Checks if the level field was explicitly provided.
   *
   * @return bool
   *   TRUE if level was provided.
   */
   ...
   * {@inheritdoc}
   *
   * @phpstan-return HeadingNodeArray
   */
  public function toArray(): array {
    ...
    /** @phpstan-var HeadingNodeArray $result */
